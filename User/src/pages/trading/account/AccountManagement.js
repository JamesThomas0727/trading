...
import { fetchAPIs, fetchTradingDatas } from '../../../utils/api';
...
export default function AccountManagement(props) {
  ...
  const { updateStopLoss, position } = useStopLoss(openPositionsData.find(position => position.id === updateID), props.bid);
  ...
  const { equity: calculatedEquity, loading: equityLoading } = useEquity(props.balance, openPositionsData);
  const { usedMargin, availableMargin } = useMargin(openPositionsData, props.balance);
  ...
  React.useEffect(() => {
    ...
    const fetchData = debounce(async () => {
      try {
        ...
        const quotes = await fetchMarketData(Symbols_total);
        if (quotes.length > 0 && isSubscribed) {
          quotes.forEach(quote => {
            const symbolIndex = props.symbols.findIndex(item => item.code === quote.symbol);
            if (symbolIndex !== -1) {
              updateAsk(symbolIndex, quote.ask);
              updateBid(symbolIndex, quote.bid);
            }
          });
        }
      } catch (error) {
  ...
  const EquityDisplay = () => (
    <div>
      ...
      {lastUpdateTime && (
        <small>Last updated: {new Date(lastUpdateTime).toLocaleTimeString()}</small>
      )}
    </div>
  );
